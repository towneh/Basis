...
namespace Basis.Media
{
    ...
    /// <see cref="BasisMediaPlayer.diagnosticsCsvPath"/>), sampled on its own
    ...
    public sealed class BasisMediaPlayerDiagnostics : MonoBehaviour
    {
        ...
        public void StartLogging()
        {
            ...
            {
                LastError = refusal;
                Debug.LogError($"[BasisMedia] diagnostics: {refusal}");
                return;
            }
            ...
            {
                LastError = e.Message;
                Debug.LogError($"[BasisMedia] diagnostics could not open '{ResolvedLogPath}': {e.Message}");
                _writer = null;
                return;
            }
        ...
        public void StopLogging()
        {
            ...
            Flush();
            try { _writer?.Dispose(); }
            catch (Exception e) { Debug.LogWarning($"[BasisMedia] diagnostics close failed: {e.Message}"); }
            _writer = null;
        }
        ...
        {
            try { _writer?.Flush(); }
            catch (Exception e) { Debug.LogWarning($"[BasisMedia] diagnostics flush failed: {e.Message}"); }
            _rowsSinceFlush = 0;
        }
        ...
        void Update()
        {
            ...
            {
                LastError = e.Message;
                Debug.LogError($"[BasisMedia] diagnostics write failed: {e.Message}");
                StopLogging();
            }
        }
        ...
        string ResolvePath(out string refusal)
        {
            ...
            if (string.IsNullOrEmpty(LogPathOverride))
                return Path.Combine(root, "BasisMediaFrames.csv");

            string full;
            try
            {
                full = Path.GetFullPath(Path.IsPathRooted(LogPathOverride)
                    ? LogPathOverride
                    : Path.Combine(root, LogPathOverride));
            }
            catch (Exception e)
            {
                refusal = $"could not resolve the log path: {e.Message}";
                return null;
            }
            ...
            {
                refusal = "the log path must sit under Application.persistentDataPath";
                return null;
            }
            return full;
        }
