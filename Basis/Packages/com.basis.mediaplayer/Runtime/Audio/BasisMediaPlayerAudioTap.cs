...
public sealed class BasisMediaPlayerAudioTap : MonoBehaviour
{
    ...
    private BasisMediaDriverTick frameTick;

    private void Awake() => frameTick = new BasisMediaDriverTick(TickSourceVolume);

    private void OnEnable() => frameTick.Arm();

    private void OnDisable() => frameTick.Disarm();

    private void Update() => frameTick.RunFromUpdate();

    private void TickSourceVolume()
    {
