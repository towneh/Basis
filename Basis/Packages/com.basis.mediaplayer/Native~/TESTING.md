...
| Seeks land on their target | A seek shows its target within a frame across picture, sound, captions and SEI, handles targets past the end, and discards no audio. A seek queued behind another supersedes it. | `cargo test -p media-engine --test session between_keyframes` + `--test session a_seek_queued`; by hand: `cargo test --release -p media-engine --test session a_long_decode_forward -- --ignored` (set `BASIS_MEDIA_TEST_SPARSE_KEYFRAMES_URL` to an H.264 MP4 over HTTPS whose last keyframe before 17 s is several seconds earlier) | CI, Windows; by hand |
...
| MP4 edit lists | A track whose edit list opens with an empty edit starts late by the gap, as ffprobe reads it, and seeks land on the shifted times; an audio track's first real edit is its encoder priming. | `cargo test -p media-demux --test mp4_stream an_empty_edit` + `cargo run -p bm-probe -- conformance fixtures` | CI |
...
| Refused tracks | An MP4 or Matroska track left out because nothing here plays it (an unknown video sample entry or CodecID, an audio track that is not AAC in MP4) is a refusal naming the codec, and the other track plays. A file with nothing else fails to open with the refusals as its reason. The engine reports each refusal as `CodecRefused` from the demux stage. | `cargo test -p media-demux --test mp4_stream refused` + `--test mkv_stream refused` + `cargo test -p media-engine --test session a_track_the_demuxer` | CI |
...
| Forced hardware fallback | With `BASIS_MEDIA_DISABLE_HW_DECODE` set, the default plays through in software and `hardware_only` gives a typed error while audio plays. With no audio, the refusal fails the session within seconds, carrying its reason. | `cargo test -p media-engine --test hw_fallback` | CI |
