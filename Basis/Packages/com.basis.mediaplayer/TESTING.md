...
| Resync keeps the owner's place | Owner presses **Resync Everyone** while playing. Run it on a direct `.mp4` and on a page URL | The owner keeps playing from where it was, **not** from zero, on both. A page URL is the harder case: it is resolved before it opens, and the session it replaces plays on for those seconds. This is the `selfResyncApply` path and the row most likely to regress; a jump to zero means the initiator's own stash is not being applied |
...
| A video hours long | Owner loads a fragmented video of two hours or more, seeks well into it, and a second client joins there. Run it twice: once on a direct URL, which takes the resolver out of it — make the file by looping a clip past two hours and remuxing it `-c copy` with `-movflags +frag_keyframe+empty_moov+default_base_moof+global_sidx`, both of which matter and the engine's `TESTING.md` says why — and once on `https://www.youtube.com/watch?v=aNS5o3VJ0-A` (11 h, H.264 the only codec it lists, so the resolver cannot pick AV1) | It opens in the time an ordinary video takes rather than minutes, the seek lands, and the joiner lands beside the owner. Length is what this row is for: the adaptive files a video site serves are fragmented, and a player that reads every fragment's header before it starts pays for all of them. On the page URL, record the date it last worked; a public link can go. Past about six hours in, expect the stream URL to expire and playback to stop — a separate problem, not this row's |
